...
/* 格式化按钮:仅编辑模式显示;批注按钮 ann-only 仅非编辑模式显示 */
.ann-toolbar .fmt-btn{display:none;background:transparent;border:none;color:#fff;cursor:pointer;font-size:12px;padding:5px 9px;border-radius:4px;line-height:1;min-width:30px;font-family:-apple-system,sans-serif;transition:background 0.15s}
.ann-toolbar .fmt-btn:hover{background:rgba(255,255,255,0.18)}
.ann-toolbar .fmt-btn.bold-style{font-weight:700}
.ann-toolbar .fmt-btn.italic-style{font-style:italic;font-family:Georgia,serif}
.ann-toolbar.edit-mode .fmt-btn{display:inline-flex;align-items:center}
.ann-toolbar.edit-mode .ann-only{display:none}
...
<button class="good ann-only" data-type="good" title="标好">👍</button>
<button class="bad ann-only" data-type="bad" title="标不好">👎</button>
<button class="suggest ann-only" data-type="suggest" title="改建议">💡</button>
<button class="question ann-only" data-type="question" title="有疑问">❓</button>
<button class="fmt-btn" data-fmt="h2" title="H2 标题">H2</button>
<button class="fmt-btn" data-fmt="h3" title="H3 小标题">H3</button>
<button class="fmt-btn" data-fmt="p" title="变回普通段落">P</button>
<span class="sep fmt-btn"></span>
<button class="fmt-btn bold-style" data-fmt="bold" title="加粗">B</button>
<button class="fmt-btn italic-style" data-fmt="italic" title="斜体">I</button>
<button class="fmt-btn" data-fmt="code" title="行内代码">{ }</button>
<span class="sep fmt-btn"></span>
<button class="fmt-btn" data-fmt="quote" title="引用">引用</button>
<button class="fmt-btn" data-fmt="ul" title="无序列表">列表</button>
<button class="fmt-btn" data-fmt="link" title="加链接">链接</button>
</div>
...
function buildToc() {
  // 清空
  tocList.innerHTML = '';
  Object.keys(tocMap).forEach(k => delete tocMap[k]);
...
// 编辑时 TOC 跟随重建(debounce 400ms,避免每键触发)
let tocRebuildTimer;
...
document.addEventListener('mouseup', (e) => {
  ...
  setTimeout(() => {
    ...
    // 编辑模式下允许选 1 字(加粗 / 单字标题);非编辑模式批注至少 3 字
    const minLen = root.getAttribute('contenteditable') === 'true' ? 1 : 3;
...
// 工具栏按钮:批注(data-type)走 dialog,格式化(data-fmt)走 applyFormat
toolbar.querySelectorAll('button').forEach(btn => {
  btn.addEventListener('click', (e) => {
    e.preventDefault();
    if (btn.dataset.type) {
      openDialog(btn.dataset.type, lastText);
      toolbar.classList.remove('show');
    } else if (btn.dataset.fmt) {
...
// 选段格式化(编辑模式下点工具栏 H2 / H3 / B / I / Code / 引用 / 列表 / 链接)
function applyFormat(fmt) {
  if (!lastRange) return;
  // 重新把 selection 还给 lastRange(点工具栏按钮可能让 focus 跑了)
  const sel = window.getSelection();
  sel.removeAllRanges();
  sel.addRange(lastRange);
  try {
    if (fmt === 'h2' || fmt === 'h3' || fmt === 'p') {
      document.execCommand('formatBlock', false, fmt.toUpperCase());
    } else if (fmt === 'quote') {
    ...
    console.warn('applyFormat error:', err);
  }
  // 触发 input 让 TOC 跟着重建 + 草稿自动保存
...
function enterEditMode() {
  ...
  root.addEventListener('input', scheduleTocRebuild);
}
...
function exitEditMode() {
  ...
  root.removeEventListener('input', scheduleTocRebuild);
  // 退出编辑后重建一次 TOC + 重新应用批注高亮
  buildToc();
  highlightAllAnnotations();
}
